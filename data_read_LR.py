...
clf=LogisticRegression(C=100, solver='lbfgs', multi_class='multinomial',max_iter=500).fit(X_train,y_train)
...
nb=LogisticRegression(C=100, solver='lbfgs', multi_class='multinomial',max_iter=500)
...
nb=LogisticRegression(C=1e5, solver='lbfgs', multi_class='multinomial',max_iter=500)
kb=StratifiedKFold(10)
...
pipeline = Pipeline([
                     ('ANOVA', SelectKBest(f_classif, k=18)),
                     ('clf',LogisticRegression(C=1, solver='lbfgs', multi_class='multinomial',max_iter=500))])
...
pipeline = Pipeline([
                     ...
                     ('clf', LogisticRegression(C=1e5, solver='lbfgs', multi_class='multinomial',max_iter=500))])
# fitting our model and save it in a pickle for later use
model = pipeline.fit(X_train, y_train)
